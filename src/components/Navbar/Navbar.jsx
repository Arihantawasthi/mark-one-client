...
function Navbar() {
    ...
    return (
        <aside
            className="w-64 fixed left-0 z-40 h-full bg-surface border-r border-border transition-all duration-300 ease-in-out flex flex-col justify-between"
        >
            <div>
            ...
            <div className="flex justify-between items-center p-4 border-t border-b border-t-border border-b-border text-sm">
                <span className="flex gap-x-2 items-center opacity-90">
                    ...
                    <span className="font-medium">{/* Username */}{ auth.username }</span>
                </span>
                <LogOut size={20} className="cursor-pointer hover:text-red-500 transition" onClick={logout} />
            </div>
        </aside>
    );
}
